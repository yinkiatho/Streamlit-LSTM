...
class AlgoTrader(QCAlgorithm):
    ...
    def Initialize(self):
        ...
        training_data = self.History(["SPY"], 252*30, Resolution.Daily)
        
    def OnData(self, data):
        
        #Display the current value of the SPY asset
        self.Debug("SPY: " + str(self.Portfolio["SPY"].TotalPortfolioValue))
    ...
    def train_LSTM(self, window=5, n_splits=5):
        ...
        params = pd.DataFrame(columns=['batch_size', 'epoch', 'score'])
        for batch_size in batch_sizes:
            for epoch in epochs:
                ...
                # Append to dataframe
                params = params.append({'batch_size': batch_size, 'epoch': epoch, 'score': Mean_Squared_Error},
                                       ignore_index=True)

        # Get Best Params individual
        params.sort_values(by=['score'], ascending=True,
